...
const Appointment = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const requestBody = {
            ...
            phone,
            date,
            service: [serviceId], // Adjust for multiple selections if needed
            stylist: employeeId,
            timeSlot,
            price,
            notes,
        ...
        try {
            const response = await axios.post(
                `${process.env.NEXT_PUBLIC_API_URL}/admin/createWalkin`, requestBody);
    ...
    const fetchAllAppointments = async () => {
        try {
            const response = await axios.get(
                ...
                `${process.env.NEXT_PUBLIC_API_URL}/admin/getAllWalkins?adminId=${adminId}`
                                        ...
                                        <tbody>
                                            {appointments && appointments.length > 0 ? (
                                                appointments.map((order, index) => {
                                                    const serviceTitle = order?.services?.title || "No Service";
                                                    const stylistName = order?.stylist?.stylistName || "No Stylist";
                                                    const categoryName = order?.categoryId?.categoryName || "No Category";

                                                    return (
                                                        <tr key={index}>
                                                            <td scope="row">{order?._id}</td>
                                                            <td className="user_td">{order?.clientName || order?.name || "No Name"}</td>
                                                            <td>{order?.date && order?.timeSlot ? `${order.date} ${order.timeSlot}` : "No Date/Time"}</td>
                                                            <td>
                                                                {serviceTitle}
                                                                <br />
                                                            ...
                                                            <td className={`status_td ${order?.status?.toLowerCase() || "unknown"}`}>
