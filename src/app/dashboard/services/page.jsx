...
import { useSelector, useDispatch } from "react-redux";
import { fetchCategories, fetchAllServices } from "../../../lib/slices/servicesSlice";
import Image from "next/image";
import Link from "next/link";
import { PiPencilSimpleLineBold } from "react-icons/pi";
import { useRouter } from "next/navigation";
...
const ServicesDashboard = () => {
  ...
  const { categories, servicesByCategory, loading, error } = useSelector((state) => state.services);
  ...
  const renderServices = (categoryId) => {
    ...
    if (services.length === 0) {
      return <p className="text-muted">No services found for this category.</p>;
    }
        ...
        <>
          ...
          {categories.map((cat) => (
            <div key={cat._id} className="mb-5">
              <h3>{cat.categoryName} Services</h3>
              <div
                className="row mt-3 flex-nowrap py-4"
                style={{ overflow: "auto", scrollBehavior: "smooth" }}
              >
                {renderServices(cat._id)}
              </div>
            </div>
          ))}
        </>
